 ...
 */
export function usePersistentChartConfig(options: UsePersistentStateOptions = {}) {
    const { autoSave = true, autoLoad = true } = options
 ...
 */
export function usePersistentColumnTypes(filename: string, options: UsePersistentStateOptions = {}) {
    const { autoSave = true, autoLoad = true } = options
    ...
    useEffect(() => {
        if (autoSave && filename && Object.keys(columnTypes).length > 0 && isStorageAvailable()) {
    ...
    return {
        ...
        updateColumnTypes,
        clearColumnTypes,
        hasStoredTypes: filename && isStorageAvailable() && loadColumnTypes(filename) !== null
    }
}
...
export function useSessionManager() {
    ...
    const hasSessionData = isStorageAvailable() && (
        loadDataset() !== null ||
