...
**Self-host deployment:** The relaunch deployment is a static frontend build packaged into a pre-built nginx Docker image, published to GHCR, and run on deployment-host behind a shared Traefik reverse proxy and a manually configured Cloudflare Tunnel, at `app.example.com`. See [docs/deployment/self-hosted.md](deployment/self-hosted.md) for full setup, update, and rollback instructions.

```text
Git push to main → CI: lint → build → test → Docker build → push to GHCR   (implemented)
GHCR image → docker compose pull/up on deployment-host                     (implemented, manual trigger)
deployment-host (Traefik) → Cloudflare Tunnel → app.example.com  (Tunnel routing configured manually, outside this repo)
```
