...
- Self-host deployment packaging (Docker/Caddy/Nginx, systemd unit, health check) for deployment-host behind Cloudflare Tunnel
...
The relaunch deployment is a static frontend build packaged into a Docker/nginx image and served from deployment-host behind a shared Traefik proxy and Cloudflare Tunnel, at `app.example.com`. Deployment artifacts (Dockerfile, nginx config, compose template, health check) exist in this repository — see [docs/deployment/self-hosted.md](docs/deployment/self-hosted.md). The stack has not yet been deployed live on deployment-host.
...
| 5 | Self-host deployment artifacts for deployment-host + Cloudflare Tunnel | Artifacts done; live deployment pending |
