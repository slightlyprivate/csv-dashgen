...
The intended relaunch deployment is a static frontend build served from deployment-host behind Cloudflare Tunnel, at `app.example.com`. Concrete deployment artifacts (Docker/Caddy/Nginx config, health check convention) do not yet exist in this repository — see the roadmap in [docs/relaunch/CODEBASE_AUDIT.md](docs/relaunch/CODEBASE_AUDIT.md).
...
| 5 | Self-host deployment artifacts for deployment-host + Cloudflare Tunnel | Not started |
